...
Loads the CatBoost .cbm file ONCE at startup and holds it in memory.
...
class YieldForecaster:
    ...
    def load(self) -> None:
        ...
        logger.info("CatBoost model loaded (best_iteration will be read from metadata)")

        with open(METADATA_PATH, "r") as f:
            self._metadata = json.load(f)
        logger.info("Metadata loaded: %s v%s", self._metadata.get("model_name"), self._metadata.get("model_id"))

        with open(FEATURE_CONFIG_PATH, "r") as f:
            self._feature_config = json.load(f)
        logger.info("Feature config loaded: %d features", len(self._feature_config.get("features", [])))
    ...
    def predict(self, request_data: dict) -> tuple[float, bool, list[str]]:
        ...
        row = {col: [request_data[col]] for col in FEATURE_ORDER}
        df = pd.DataFrame(row)

        # Run CatBoost inference
        raw_prediction = float(self._model.predict(df)[0])

        # Enforce non-negative constraint (model metadata note)
