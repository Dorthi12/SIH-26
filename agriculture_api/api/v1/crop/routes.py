"""
crop_routes.py — Recommendation endpoints for the Crop Recommendation Engine.
...
def _validate_location(state: str, district: str, season: str):
    """Validate state → district → season chain. Raise 400/404 with meaningful errors."""
    if crop_artifacts.resolve_state(state) is None:
        raise HTTPException(
            status_code=404,
            detail={
                "code": "INVALID_STATE",
                "message": f"State '{state}' is not available. "
    ...
    if crop_artifacts.resolve_season(season) is None:
        raise HTTPException(
            ...
            detail={
                "code": "INVALID_SEASON",
                "message": f"Season '{season}' is not supported. "
...
def score_crop(req: ScoreCropRequest):
    ...
    if crop_rows.empty:
        ...
        return ScoreCropResponse(
            ...
            found=False,
            message=(
                f"Crop '{req.crop}' has no historical data for this location/season."
            ),
            available_crops=available,
        )

    # Build ranked crop at correct rank position
    crop_rank = rank_within(candidates, req.crop)
    ranked = rank_crops(crop_rows, top_k=1)
    ranked[0].rank = crop_rank   # override rank with position among all candidates
...
@router.post(
    ...
    description=(
        "Same as `/score` but the response emphasises the `score_breakdown` "
        "field showing how yield, stability, and experience individually contribute "
        "to the final `historical_score`. "
        "Useful for the agent to generate human-readable explanations:\n\n"
        "_'Rice ranks first because it has strong historical yield performance '_ "
        "_'(contributes 45% of the score) and 18 years of cultivation history.'_"
    ),
)
